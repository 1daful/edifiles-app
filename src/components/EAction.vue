<template>
  <QBtn :id="action.id" :label="action.label?.text" @click="event()" :[type]="true" :[shape]="true" 
  ...
  <QBtn :id="action.id" :label="typeof action.label==='string' ? action.label : action.label?.text" :icon="action.icon" v-else-if="component && show">
    <QPopupProxy cover>
...
import { Action, View } from "../utils/DataTypes";
...
let label = () => {
  if (typeof props.action.label === 'object') {
    return props.action.label
  }
  
  else {
    return { label: props.action.label }
  }
}
