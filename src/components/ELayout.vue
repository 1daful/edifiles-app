  ...
  <!--div class="row jstify-start">
    <div class="col-3 header-link">
      <QList :bordered="style?.border" :dense="style?.dense" :dark="style?.dark">
        <EComponent :item="item" v-for="item in viewSections"></EComponent>
        <slot name="nav"></slot>
      </QList>
    </div>
  </div-->

  <q-layout class="hero-header col" :style="heroStyle" style="min-height: 50px" view="'lHh Lpr lFf'">
    <q-header
    ...
    >
      <q-toolbar class="text-h5 text-weight-bolder row justify-between">
        ...
        <div  class="gt-sm">
          <EComponent :item="item" v-for="item in pageView.header"></EComponent>
        </div>
        <slot name="nav"></slot>
        <q-btn
          size="20px"
    ...
    <q-page-container class="bg-grey-2">
      <q-page>
        ...
        <EView :view="view" v-for="view in pageView.main"></EView>
      </q-page>
      <q-page-scroller>
